...
contract ArdCoin_Factory { //contract factory pattern
    address[] public deployedArdCoins;

    function createArdCoin() public {
        address newArdCoin = address (new ArdCoin (msg.sender));
        deployedArdCoins.push(newArdCoin);
    }

    function getDeployedArdCoin() public view returns (address[] memory) {
        return deployedArdCoins;
    }
}
...
contract ArdCoin is ERC20Interface, Ownable {
    ...
    constructor(string memory _symbol, string memory _name, uint _supply, uint8 _decimals) public {
        tData.symbol = _symbol;
        tData.name = _name;
        tData.decimals = _decimals;
        //symbol = _symbol;
        //name = _name;
        //decimals = _decimals;
        totalSupply_ = _supply * 10**uint(tData.decimals);
    ...
    // ------------------------------------------------------------------------
    function burn(uint _value) public noReentrancy {
