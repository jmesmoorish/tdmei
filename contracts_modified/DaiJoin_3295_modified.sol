...
contract ETHJoin is LibNote {
    ...
    function exit(address payable usr, uint wad) external note {
        ...
        require(msg.sender == address(usr), "caller must be account"); // checks-effects-interaction pattern
        vat.slip(ilk, msg.sender, -int(wad)); // original line 181
        usr.transfer(wad);
    }
}
...
contract DaiJoin is LibNote {
    ...
    modifier noReentrancy() { //mutex pattern
        require(!locked);
        locked = true;
        _;
        locked = false;
    ...
    // --- Auth ---
    mapping (address => uint) public wards;
    function rely(address usr) external note auth noReentrancy { wards[usr] = 1; } // original line 189 //mutex pattern
    function deny(address usr) external note auth { wards[usr] = 0; }
    modifier auth {
