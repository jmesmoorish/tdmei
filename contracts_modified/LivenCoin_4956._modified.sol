...
contract LivenCoin_Factory { //contract factory pattern
    address[] public deployedLivenCoins;

    function createLivenCoin() public {
        address newLivenCoin = address (new LivenCoin (msg.sender));
        deployedLivenCoins.push(newLivenCoin);
    }

    function getDeployedLivenCoin() public view returns (address[] memory) {
        return deployedLivenCoins;
    }
}
...
contract LivenCoin is ERC20, Ownable {
    ...
    uint256 public initialAmount = 10000000000 * (10 ** tData.decimals);
    ...
    modifier afterUnlock() { // mutex pattern
    ...
    function transfer(address _to, uint256 _value) public afterUnlock returns (bool) {
        require(_value > 0, 'Token units cannot be 0!'); //guard check pattern
    ...
    function transferFrom(address _from, address _to, uint256 _value) public afterUnlock returns (bool) {
        require(_value <= balances_[_from], "Value exceeds balance of msg.sender."); //check-effects-interaction pattern
