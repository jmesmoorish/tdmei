...
contract FaucetPay_Factory { //contract factory pattern
    address[] public deployedFaucetPays;

    function createFaucetPay() public {
        address newFaucetPay = address (new FaucetPay ());
        deployedFaucetPays.push(newFaucetPay);
    }

    function getDeployedFaucetPay() public view returns (address[] memory) {
        return deployedFaucetPays;
    }
}
...
contract FaucetPay {
    ...
    modifier _onlyOwner(){
     ...
     function destroy() public _onlyOwner { // mortal pattern
        selfdestruct(adminAddress);
    }

    function destroyContract(address payable recipient) public _onlyOwner { // mortal pattern
