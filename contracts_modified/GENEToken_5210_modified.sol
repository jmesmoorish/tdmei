...
contract GENEToken_Factory { //contract factory pattern
    address[] public deployedGENETokens;

    function createGENEToken() public {
        address newGENEToken = address (new GENEToken (0x0000000000000000000000000000000000000000,1000000000));
        deployedGENETokens.push(newGENEToken);
    }

    function getDeployeGENEToken() public view returns (address[] memory) {
        return deployedGENETokens;
    }
}

contract GENEToken is StandardToken, Owned {
    // metadata
    ...
    function formatDecimals(uint256 _value) internal pure returns (uint256 ) {
        return _value * 10 ** uint256(tData.decimals);
    }
    ...
    function startFunding (uint256 _fundingStartBlock, uint256 _fundingStopBlock) isOwner external {
        require(!isFunding); //guard check pattern
    ...
    /// 转账ETH 到 GE 团队
    function transferETH() isOwner noReentrancy external {
    ...
    function allocateToken (address _addr, uint256 _eth) isOwner external {
        require(_eth != 0); //check-effects-interaction pattern
